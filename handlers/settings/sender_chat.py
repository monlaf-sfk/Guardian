...
router=Router()
@router.message(Command('sender_chat'))
async def sender_chat_handler(message: types.Message, session: Session):
    ...
    if chat.permission:
        ...
        if user.status == "creator":
            ...
            if len(arg) ==0:
                ...
                if settings.sender_chat:
                    settings.sender_chat = False
                    await message.reply('✅ Писать от имени каналов Разрешены!')

                else:
                    settings.sender_chat = True
                    await message.reply('❌ Писать от имени каналов Запрещено!')
                session.commit()
                ...
                punishtime.time_sender_chat = restriction_time
                session.commit()
