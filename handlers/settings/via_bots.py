...
router=Router()
@router.message(Command('via_bots'))
async def via_bots_handler(message: types.Message, session: Session):
        ...
        if user.status == "creator":
            ...
            if len(arg) ==0:
                ...
                if settings.via_bots:
                    settings.via_bots = False
                    await message.reply('✅ VIA Bots Разрешены!')

                else:
                    settings.via_bots = True
                    await message.reply('❌ VIA Bots Запрещены!')
                session.commit()
                ...
                punishtime.time_via_bots = restriction_time
                session.commit()
